 ...
 * fail <client> A <from> <till> Availability: check which car types are
 * available from <from> until <till>. <client> B <from> <till> <car type> Book:
 * request a quote for a client named <client>, for a car of type <car type>
 * from <from> until <till>. <client> F Finalize: finalize the quote made for
 * client <client> <client> MR Manager, Reservations: print all reservations of
 * client <client> <client> M <type:nr>* Manager: check total number of
 * reservations per car type, each type <type> has <nr> reservations
 */
public abstract class AbstractScriptedSimpleTest {
	 ...
	 */
	protected abstract void checkForAvailableCarTypes(Date start, Date end)
			throws Exception;
	 ...
	 *             if things go wrong, throw exception
	 */
	protected abstract Quote createQuote(String clientName, Date start,
			Date end, String carType) throws Exception;
	 ...
	 *             if things go wrong, throw exception
	 */
	protected abstract List<Reservation> getReservationsBy(String clientName)
			throws Exception;
	 ...
	 *             if things go wrong, throw exception
	 */
	protected abstract int getNumberOfReservationsForCarType(String carType)
			throws Exception;
			...
			while (in.ready()) {
				...
				if (cmd.contains("M")) {
					...
					if (be == null && shouldfail)
						System.err.println("command should have failed: "
								+ line + " on line " + lnr);
					if (be != null && !shouldfail) {
					...
					if (reservation == null)
						throw new IllegalArgumentException(
								"script broken: no quote" + line + " on line "
										+ lnr);
					Exception be = null;
					try {
	...
	private void check(String name, StringTokenizer scriptReader)
			throws Exception {
		while (scriptReader.hasMoreTokens()) {
			...
			if (Integer.parseInt(pair[1]) == nr) {
				System.out.println(name + " has correct totals " + pars + " "
						+ nr);
			} else {
				System.err.println(name + " has wrong totals " + pars + " "
						+ nr);
			}
		}
	}
